...
def load_mediapipe():
    """
    Fixed MediaPipe loading with proper error handling
    """
    try:
        ...
        if not hasattr(mp, 'solutions'):
            st.error("⚠️ MediaPipe installation issue detected. Please reinstall mediapipe.")
            return None, None
        ...
        st.info("Please add 'mediapipe' to requirements.txt")
        return None, None
    except AttributeError as e:
        st.error(f"MediaPipe version issue: {e}")
        st.info("Try specifying mediapipe==0.10.9 in requirements.txt")
        return None, None
    except Exception as e:
        st.error(f"Error loading MediaPipe: {e}")
        st.info("Check that opencv-python and protobuf are also installed")
        return None, None
...
if not st.session_state.models_ready:
    with st.spinner("🚀 Initializing AI Models..."):
        ...
        if face_detection is None or face_mesh is None:
            st.error("❌ Failed to load MediaPipe models. Please check the error messages above.")
            st.info("""
            **Troubleshooting Steps:**
            1. Create a `requirements.txt` file with:
               ...
               onnxruntime
               ```
            2. On Streamlit Cloud, go to App Settings → Python version (use 3.10 or 3.11)
            3. Redeploy the app
            """)
            st.stop()
        else:
